...
/**
 *  
 * This is a tester class that test all the different method of the game
 * @author a
 *
 */
public class CarrotPatchTester extends CarrotPatch {
  ...
  public static boolean test2isCloseMethod() {
    ...
    try {
      ...
      Wolf w1 = new Wolf();
      Wolf w2 = new Wolf();
      // Set Rabbit at position(400,400)
      r.setX(400);
      r.setY(400);
      // Set first Wolf at position(200,400)
      w1.setX(200); // Wolf is 200px away from Rabbit
      w1.setY(400);
      // Set second Wolf at position(300,200)
      w2.setX(300); // Wolf is 100px away from Rabbit
      w2.setY(400);
  ...
  public static boolean test3isCloseMethod() {
    ...
    try {
      ...
      w2.setX(300); // Wolf is 100px away from Rabbit
      w2.setY(400);
      if (r.isClose(w1.getX(), w1.getY(), Rabbit.getScanRange())) { // bug! isClose() should return
                                                                    // false here
        System.out.println(
            "Problem detected: Rabbit.isClose() returned true when it should return false.");
        return false;
      }
      if (!r.isClose(w2.getX(), w2.getY(), Rabbit.getScanRange())) { // bug! isClose() should return
                                                                     // true here
        System.out.println(
            "Problem detected: Rabbit.isClose() returned false when it should return true.");
        return false;
